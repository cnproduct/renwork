...
const outputDir = resolve(process.env.RUNNER_TEMP || ".", "renwork-electron-manifests");
...
function isReleaseAsset(path) {
  ...
  if (!basename(path).startsWith("renwork-")) return false;
...
function validateManifest(name, manifest) {
  ...
  const expectedPrefix = distribution === "public" ? "renwork-" : `renwork-${distribution}-`;
  if (
    urls.some((url) => !url.startsWith(expectedPrefix))
    || (distribution === "public" && urls.some((url) => /^renwork-(?:cloud|enterprise)-/.test(url)))
  ) {
