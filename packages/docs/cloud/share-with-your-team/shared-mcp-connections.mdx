...
description: "Share Notion, Linear, Stripe, Exa, or any MCP server in RenWork Cloud with Individual accounts by default, or One org account when everyone should act as the same account"
---
...
## Publish the connection in RenWork Cloud

1. Open `MCP Connections` in the RenWork Cloud dashboard.
...
<Frame>
  ![MCP Connections admin screen in RenWork Cloud](/images/cloud-mcp-connections-admin.png)
</Frame>
...
For the hosted RenWork Cloud instance, the dashboard is
...
3. In RenWork Cloud, add the Slack connection and paste the app's client ID
   and client secret.
...
1. Sign in to RenWork, then open `Settings` > `Connect`.
...
4. The browser shows `Connected` — return to RenWork.
5. The connection updates by itself, no reload needed, and moves to
   `Ready to use`.

Your sign-in is stored in RenWork Cloud, not on this machine — connect once
...
The agent searches the live capabilities available through RenWork Connect,
...
services managed through RenWork Cloud.
