...
description: "Provision RenWork members and teams from Microsoft Entra ID"
---

Use Microsoft Entra ID provisioning to create, update, and deactivate RenWork
members automatically. Entra security groups can also become RenWork teams
...
- An RenWork organization with Enterprise SSO and SCIM.
- An RenWork owner, or a member who can manage security configuration.
- A working RenWork SAML connection. Complete and test
...
sure at least one RenWork owner can sign in through SAML before relying on
Entra as the identity source.

## 1. Get the RenWork connection values

In RenWork, select the organization you want to connect, then open
**Settings → SCIM**.
...
3. Copy the **SCIM bearer token** immediately. RenWork shows the full token
   only after creating or rotating it.
4. To synchronize Entra groups as RenWork teams, select **Enable team sync**
...
| --- | --- |
| Tenant URL | RenWork **Settings → SCIM → SCIM base URL**. It normally ends in `/api/auth/scim/v2`. |
...
2. Select the same enterprise application used for RenWork SAML SSO.
3. Open **Provisioning**.
...
6. Paste the RenWork **SCIM base URL** into **Tenant URL**.
7. Paste the RenWork **SCIM bearer token** into **Secret Token**.
...
principal names use an `onmicrosoft.com` domain but users sign in to RenWork
...
create a second RenWork member for the same person.
...
not enough; assign it to the RenWork enterprise application as well.
...
3. Confirm that import, scope evaluation, matching, and the final action all
   succeed.
4. In RenWork, open **Members** and confirm that the member appears with the
   expected email.
...
6. In RenWork, open **Members → Teams** and confirm that the group appears as a
...
sends a group-create request to RenWork. Add members and select them explicitly
...
5. Refresh RenWork **Members** and **Teams** to verify the final state.
...
1. Remove a test user from the Entra group and confirm that RenWork removes the
   user from the SCIM-managed team.
2. Disable or unassign a test user and confirm the expected RenWork
   deprovisioning behavior.
3. Add another assigned group and confirm that RenWork creates a second
   SCIM-managed team.

Change SCIM-managed teams in Entra, not in RenWork. RenWork keeps manually
...
| --- | --- | --- |
| **Test Connection** fails | Incorrect Tenant URL, expired or rotated token, or copied whitespace | Copy the base URL and a current token again from RenWork, then retest |
...
| On-demand group provisioning reports an internal server error without an RenWork request | The group is empty or no members were selected for the on-demand run | Add members, select up to five under **Selected users**, and retry |
| A group provisions but no RenWork team appears | RenWork team synchronization is disabled | Open **Settings → SCIM** and enable **Create teams from SCIM groups** |
| Entra creates a duplicate RenWork member | SAML and SCIM use different email identifiers | Align the SAML NameID and the SCIM `userName` mapping |
| Entra reports success but RenWork still shows the old state | The UI or incremental cycle has not refreshed | Refresh RenWork, check Entra **Provisioning logs**, and review the SCIM health status in RenWork |
