---
title: "Get started with RenWork Cloud"
description: "Connect the desktop app to your RenWork Cloud org and unlock shared workspaces, marketplaces, and shared providers"
---

RenWork Cloud requires an active subscription at [app.openworklabs.com](https://app.openworklabs.com/).
...
<Frame>
  ![RenWork Cloud org settings with organization identity, allowed email domains, and desktop restrictions](/images/cloud-org-settings.png)
</Frame>

1. Open [app.openworklabs.com](https://app.openworklabs.com/) in your browser.
2. Click `Create organization`.
3. Enter your org name.
4. Finish checkout if RenWork Cloud asks for billing.
...
4. If the browser does not return to RenWork automatically, click `Paste sign-in code`, paste the `openwork://den-auth?...` link or one-time code from RenWork Cloud, then click `Finish sign-in`.
...
- RenWork Cloud MCP requests go to `<baseUrl>/api/den/mcp/...`.
...
`Settings -> Cloud`, RenWork writes the new `baseUrl` back to the bootstrap
...
- Use [Shared workspaces](/cloud/run-in-the-cloud/shared-workspace) to deploy hosted RenWork runtimes that teammates can open from the desktop app.
