---
title: "RenWork MCP for Cursor"
sidebarTitle: "Cursor"
description: "Cursor MCP setup for RenWork Connect on Cursor Desktop, Cursor Web, and Cursor Agents."
---

The RenWork MCP server ([RenWork Connect](/cloud/run-in-the-cloud/cloud-mcp))
enables Cursor to use the RenWork capabilities your
...
Linear, Slack, Stripe, Exa, and Context7, shared skills, and RenWork Cloud
...
OAuth sign-in uses your RenWork account, and the organization you pick is
pinned into the token. If you do not have an account, create a
[free RenWork account](https://app.openworklabs.com?mode=sign-up).

## Cursor Desktop

Add RenWork as a remote MCP server in Cursor's MCP settings, paste the server
URL, and start OAuth from Cursor's connection prompt. Sign in to RenWork and
pick the organization Cursor should use.

Cursor Desktop's OAuth callback is the private-use URI
`cursor://anysphere.cursor-mcp/oauth/callback`. RenWork accepts exactly this
...
Other private-use callback schemes remain rejected: RenWork otherwise accepts
...
RenWork, and pick the organization Cursor should use.

Cursor Web and Cursor Agents use HTTPS OAuth callbacks, which RenWork
accepts.
...
steps match RenWork Connect's OAuth profile: HTTPS or HTTP loopback callbacks
...
- "Which RenWork organization am I connected to?"
