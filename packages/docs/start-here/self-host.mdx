---
title: "Self-host"
description: "Run RenWork on your own server or infrastructure."
---

Most users just use the [desktop app](https://openworklabs.com/download). Self-hosting is for teams that want RenWork to run on their own server, VPC, or private cloud and then connect the desktop app to it remotely.

Want hosted workspaces, team marketplaces, or shared providers without running infra? Use [RenWork Cloud](/cloud/get-started) instead. Rolling this out across a company? [Book a call](https://calendar.app.google/86QpCENvhfEzDFLu5) about [Enterprise](/cloud/enterprise).

## What you deploy

RenWork is split into normal services:

- **RenWork app**: the desktop or web client people use.
...
- **Inference service**: optional RenWork Models proxy and metering service.

If you only need one private remote workspace, deploy the worker runtime. If you want RenWork Cloud-style accounts, teams, and organization management, deploy Den web, Den controller, and optionally the inference service.
...
- Hosted RenWork uses PlanetScale for Den control-plane state. PlanetScale production clusters provide one primary and at least two replicas across three availability zones, managed failover, and automated backups.
...
- Set a unique `DEN_DB_ENCRYPTION_KEY` of at least 32 characters. RenWork uses it to encrypt selected sensitive database columns, but it does not replace infrastructure-level encryption at rest.
...
### RenWork Cloud and the desktop app

RenWork cloud-style self-hosting is composed of three main pieces:
...
RenWork does not require many third-party services for the core runtime. These are optional or deployment-specific:
...
Skip the server. [**RenWork Cloud**](/cloud/get-started) runs hosted workers for your team.
...
If you're rolling RenWork out across a company of 150\+ employees, a single self-hosted server VM might be harder to manage.
