...
- A connected service such as ServiceNow appears ready in RenWork Cloud, but the task does not expose `openwork-cloud_search_capabilities` or `openwork-cloud_execute_capability`.

**What this diagnostic checks**

The prompt below runs RenWork's bundled OpenCode executable against its generated runtime config and checks whether a fresh OpenCode process can connect to `openwork-cloud`. It does not attach to the OpenCode server already running inside RenWork, so it combines that result with the live status shown in RenWork and a forced tool call from a new task.
