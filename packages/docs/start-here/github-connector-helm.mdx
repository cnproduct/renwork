...
description: "Create a GitHub App and configure the RenWork EE Helm chart for repository connectors."
---

The GitHub connector lets a self-hosted RenWork deployment install a GitHub App, list repositories the installation can read, discover plugin or marketplace manifests, import selected repositories, and keep imported content fresh from GitHub webhooks.
...
The GitHub App needs these RenWork URLs:
...
## What RenWork does with it

When an admin clicks **Connect GitHub** in RenWork:
...
Choose GitHub, install the GitHub App on a test account or organization, select a repository, and start discovery. A working setup should return to RenWork, show repositories visible to the installation, and allow discovery on a selected repository.
