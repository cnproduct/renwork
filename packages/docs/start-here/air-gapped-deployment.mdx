...
description: "Plan RenWork deployments that limit or remove public internet access."
---

"Air-gapped" can mean several different things in RenWork planning. Use these terms precisely so the network design, installer path, and feature expectations stay aligned.
...
| **Fully air-gapped installer delivery** | Only the installer bytes are mounted or served from inside your network. This does **not** make the entire RenWork product fully air-gapped. It only removes the GitHub release-asset download path for the installer. |
| **Fully isolated RenWork deployment** | Every dependency used by the enabled RenWork features is mirrored, provisioned, disabled, or replaced with an internally reachable service. This includes deployment artifacts, desktop runtime dependencies, model/provider catalogs, OAuth/provider endpoints, MCP servers, email, diagnostics, updates, and trust anchors. |
...
Use this checklist only when the goal is a fully isolated RenWork deployment, not just private Den or internal installer delivery.
...
- Point `OPENCODE_MODELS_URL` at an internal mirror of the model catalog. The default catalog is `https://models.dev/`.
- The packaged desktop can start the UI-control MCP with `npx -y openwork-ui-mcp`. In a fully isolated environment, provide an internal npm registry or npm mirror that can serve that package to `npx`. RenWork does not currently document a separate RenWork-specific registry override for this package; use standard npm configuration for the runtime account.
