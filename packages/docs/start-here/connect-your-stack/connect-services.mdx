...
description: "Sign in to RenWork and connect the accounts your agent can use"
---

Use `Settings` > `RenWork Connect` for services your organization makes available,
...
Connect requires signing in to an RenWork account and joining an organization
that has Connect enabled. This is separate from signing in to an LLM provider
such as ChatGPT.

1. Open the RenWork desktop app.
2. Click `Sign in`, or open `Settings` > `RenWork Connect` and choose `Sign in`.
3. Complete sign-in in your browser and return to RenWork.
4. Choose the RenWork organization you want to use if prompted.
...
send you back to `Settings` > `RenWork Connect` with the action to take instead of
...
- **Connections** is the organization-admin dashboard in RenWork Cloud for
  publishing a service, choosing whose account it uses, and granting member or
  team access.
- **RenWork Connect MCP** is the hosted endpoint that lets external MCP
  clients use capabilities from an RenWork organization.
