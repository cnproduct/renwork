...
description: "Connect a custom or local MCP server that is not available through RenWork Connect"
---

Use this guide only when you need a custom or local MCP server that is not available through RenWork Connect.
...
Connecting a custom app with RenWork takes four clicks.
...
back to RenWork. If a custom MCP needs behavior RenWork does not support,
