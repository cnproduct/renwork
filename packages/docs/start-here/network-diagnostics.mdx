...
description: "Diagnose DNS, TLS, proxy, catalog, and Den outbound issues in locked-down RenWork deployments."
---

Use this page when RenWork reaches the wrong server, cannot complete TLS, is blocked by a proxy, or needs proof that Den can reach an approved diagnostic origin. For non-network runtime, MCP, and stale-tool problems, use [Diagnostic prompts](/start-here/troubleshooting/diagnostic-prompts).
...
| --- | --- | --- |
| `LIKELY TLS INTERCEPTION` | The leaf issuer is an internal or proxy CA instead of the expected public issuer. | Allowlist the RenWork hosts through the proxy, or install the enterprise root in every RenWork trust surface that needs it. |
...
Settings → Debug includes a cloud catalog diagnostic that probes the managed `renwork-cloud` MCP entry and verifies that Cloud exposes exactly `search_capabilities` and `execute_capability`.

For self-hosted Den origins, set `OPENWORK_AGENT_DIAGNOSTICS_TRUSTED_ORIGINS` before RenWork launches:
