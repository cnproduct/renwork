...
- `models.dev` is the default OpenCode model catalog. Set `OPENCODE_MODELS_URL` to an approved internal mirror if the public catalog is blocked.

## Desktop and local runtime destinations

| Host | Requirement | Why it is reached | What breaks when blocked |
| --- | --- | --- | --- |
| `www.rrenn.com` | Required for RenWork Cloud | Hosted Cloud web origin plus same-origin Den API/MCP routes under `/api/den/...`. | Cloud sign-in, org settings, marketplace, and remote workspace flows cannot load. |
| Your self-hosted Den web origin | Required for self-hosted desktop | Replaces `www.rrenn.com` for signed-in desktop traffic. In the single-origin shape, API and MCP use `/api/den/...` on the same origin. | The desktop cannot sign in or reach Den. |
...
| `models.dev` | Required in practice unless mirrored | Default OpenCode model catalog passed as `OPENCODE_MODELS_URL`. | Model selection can be stale, incomplete, or unavailable. |
...
| `rrenn.com`, `openwork.dev`, `discord.gg`, `ollama.com`, `opencode.ai`, `linear.app`, `mail.google.com` | Optional or link-only | Docs, feedback, community, provider help, local model links, sample links, and generated Gmail links. | Links do not open; core app behavior is unaffected. |
