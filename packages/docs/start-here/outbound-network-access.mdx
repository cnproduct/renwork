...
description: "Customer IT allowlist guidance for RenWork desktop, Den, deployment artifacts, and opt-in providers."
---

RenWork works best when customer IT approves outbound access by component. The canonical machine-readable source is [`docs/enterprise/outbound-access.json`](https://github.com/cnproduct/renwork/blob/dev/docs/enterprise/outbound-access.json). CI guards that file with `node scripts/check-outbound-access.mjs`; this page is the published human-readable summary.

## Minimum hosted desktop allowlist

For a practical RenWork Cloud desktop install, allow TCP 443 to these hosts:
...
| --- | --- | --- | --- |
| `account.rrenn.com` | Required for RenWork Cloud | Hosted account, passwordless sign-in, organization selection, and desktop handoff origin. | Users cannot register, sign in, select an organization, or connect the desktop. |
| `app.openworklabs.com` | Legacy hosted Cloud origin | Retained for older configurations while current RenWork desktops use `account.rrenn.com`. | Legacy hosted Cloud configurations cannot load. |
| `api.openworklabs.com` | Required for hosted install and RenWork Connect | Hosted public API and RenWork Connect MCP endpoint used by install and external-client flows. | Hosted install checks and public RenWork Connect fail. |
...
| `registry.npmjs.org` | Required in practice unless npm is mirrored | `npx` package resolution for `openwork-ui-mcp` and development-only helper commands. | RenWork UI-control MCP is unavailable. |
...
| `rrenn.com`, `openworklabs.com`, `openwork.dev`, `discord.gg`, `ollama.com`, `opencode.ai`, `linear.app`, `mail.google.com` | Optional or link-only | RenWork website, legacy docs, feedback, community, provider help, local model links, sample links, and generated Gmail links. | Links do not open; core app behavior is unaffected. |

`code.claude.com` and `schemas.agentskills.io` appear in source or schema metadata. They are findable in the manifest, but RenWork does not fetch them for normal runtime behavior.
...
| Host | Requirement | Why it is reached | What breaks when blocked |
| --- | --- | --- | --- |
| `ghcr.io` | Required unless mirrored | RenWork Helm chart and container images. | Kubernetes cannot pull the chart or `openwork-den-api`, `openwork-den-web`, and optional `openwork-inference` images. |
...
| Custom model providers | `api.siliconflow.cn`, `openrouter.ai`, or the customer-configured provider URL | Only model discovery and inference for the enabled custom provider fail. |
