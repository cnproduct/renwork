...
const appFixture = join(sourceDir, "RenWork.app")
const installDir = join(temp, "install")
const binDir = join(temp, "bin")
const appDir = join(temp, "Applications")
const dmgPath = join(temp, "RenWork.dmg")
...
<key>CFBundleExecutable</key><string>RenWork</string>
<key>CFBundleIdentifier</key><string>com.openwork.fixture</string>
<key>CFBundleName</key><string>RenWork</string>
<key>CFBundlePackageType</key><string>APPL</string>
<key>CFBundleShortVersionString</key><string>0.0.0-fixture</string>
</dict></plist>
`)
  writeFileSync(join(appFixture, "Contents", "MacOS", "RenWork"), "#!/usr/bin/env sh\necho RenWork fixture\n")
  chmodSync(join(appFixture, "Contents", "MacOS", "RenWork"), 0o755)

  execFileSync("hdiutil", ["create", "-quiet", "-volname", "RenWork", "-srcfolder", sourceDir, "-ov", "-format", "UDZO", dmgPath])
  ...
  prove("A real macOS DMG install manifest is available", {
    action: "Create RenWork.app fixture, package it with hdiutil, and write manifest JSON",
  ...
  const appPath = join(appDir, "RenWork.app")
  prove("The installed CLI can download, verify, mount, and install RenWork.app from a DMG", {
    action: "openwork install app --manifest <fixture-manifest> --app-dir <tmp>/Applications --json",
    assert: "exit 0, checksum recorded, and RenWork.app copied into app dir",
...
<title>RenWork App DMG Install — fraimz</title>
...
</head><body><main><h1>RenWork App DMG Install — fraimz</h1><div class="meta">Result: <code>${allOk ? "passed" : "failed"}</code> · Frames: ${frames.length}</div>
