...
try {
  ...
  await withStubDenApi(
    () => ({
      ...
      skill: { id: "cob_test", title: "First RenWork Skill", output: "OPENWORK_BOOTSTRAP_SKILL_TRIGGERED" },
  ...
  await withStubDenApi(
    ...
    async (baseUrl) => {
      ...
      writeFileSync(
        ...
        `${JSON.stringify({ brandAppName: "RenWork Demo A" })}\n`,
        "utf8",
      )
      ...
      assert.equal(writtenExplicit.brandAppName, "RenWork Demo A", "prepared desktop setup must preserve its profile identity")
    },
  )
