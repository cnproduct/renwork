...
type PersistedConnectState = {
  ...
  updatedAt: number;
  /**
   * Server-scoped RenWork Connect (`renwork-cloud`) MCP desired config.
...
export function googleWorkspaceConnectGuidance(cloudHealthOrReady: CloudMcpHealth | boolean | null): string {
  ...
  if (usable) {
    return "Google Workspace is available through the RenWork Cloud connection: call search_capabilities to find the capability, then execute_capability to run it. Do not tell the user to reconfigure extensions; the relevant settings surface is Settings > Connect.";
  }
  if (cloudHealthOrReady && typeof cloudHealthOrReady !== "boolean" && cloudHealthOrReady.desired.present) {
    ...
    return `Google Workspace is connected through RenWork Connect, but agent access needs attention for this workspace. Direct the user to Settings > Connect.${suffix}`;
  }
...
/** Read the host-level renwork-cloud MCP config used for Connect catalog/skills. */
...
/** Persist the host-level renwork-cloud MCP config (server-scoped Connect). */
...
async function inspectConnectRuntime(
  ...
  for (const workspace of config.workspaces) {
    ...
    const mcp = runtimeMcpMap(inspection.config);
    if (Object.hasOwn(mcp, RENWORK_CLOUD_MCP_NAME) || Object.hasOwn(mcp, LEGACY_OPENWORK_CLOUD_MCP_NAME)) {
