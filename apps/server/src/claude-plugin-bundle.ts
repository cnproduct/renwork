...
export async function resolveClaudePluginBundle(input: { url: string; ref?: string }): Promise<ClaudePluginBundle> {
  ...
  if (manifest.hooks !== undefined) {
    warnings.push("This plugin declares hooks, which RenWork does not support yet. Hooks were skipped.");
  }
  ...
  const addMcpServers = (value: unknown) => {
    ...
    for (const [name, config] of Object.entries(record)) {
      ...
      if (mcpConfigReferencesPluginRoot(config)) {
        warnings.push(`MCP server "${name}" uses \${CLAUDE_PLUGIN_ROOT} (a plugin-local command), which RenWork does not support yet. It was skipped.`);
        continue;
      }
  ...
  if (memberships.length === 0) {
    throw new ApiError(400, "plugin_empty", "This plugin has no MCP servers, skills, commands, or agents RenWork can install.");
  }
