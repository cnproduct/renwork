...
export function createWorkspaceStore({
  ...
  async function fetchOpenworkWorkspaceList(hostUrl, token, hostToken) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`RenWork workspace discovery failed (${response.status} ${response.statusText || "HTTP error"})`);
      }
  ...
  async function readWorkspaceState() {
    ...
    if (!workspaceStateExists && process.env.OPENWORK_DESKTOP_DISABLE_WORKSPACE_RECOVERY !== "1") {
      ...
      if (recoveredWorkspaces.length > 0) {
        ...
        console.info("[migration] recovered desktop workspaces from persisted RenWork state", {
    ...
    // Older desktop state can contain multiple RenWork remote entries that
