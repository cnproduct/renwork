...
export async function waitForFreshMcpAuthorization(input: {
  ...
  while (now() - startedAt < timeoutMs) {
    if (!input.isScopeCurrent()) {
      throw new Error("The active RenWork Cloud account changed while reconnecting. Try again in this workspace.")
    }
    try {
      ...
      if (error instanceof Error && error.message.startsWith("The active RenWork Cloud account changed")) throw error
