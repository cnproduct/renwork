...
function remoteSupportMessage(message: string) {
  return `${message} Upgrade the RenWork host and try again. If this continues, contact team@openworklabs.com.`;
}
...
export function resolveRemoteWorkspaceConnectionTarget(workspace: WorkspaceInfo): TargetResult {
  ...
  if (workspace.remoteType && workspace.remoteType !== "openwork") {
    return {
      ...
      state: {
        status: "error",
        message: "Connection diagnostics are only available for RenWork remote workers.",
        checkedAt: Date.now(),
      },
    };
  }
...
export async function testRemoteWorkspaceConnection(
  ...
  if (!target.token) {
    return fail(
      remoteSupportMessage(`Token is missing for ${target.endpointLabel}. Edit connection and paste a valid RenWork token.`),
      checkedAt,
    );
  }
