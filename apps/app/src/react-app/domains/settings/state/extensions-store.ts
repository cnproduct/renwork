...
export function createExtensionsStore(options: {
  ...
  const persistImportedCloudMarketplaces = async (nextMarketplaces: Record<string, CloudImportedMarketplace>) => {
    ...
    if (!persisted) {
      throw new Error("RenWork server unavailable. Connect to manage imported cloud marketplaces.");
    }
  ...
  const persistImportedCloudPlugins = async (nextPlugins: Record<string, CloudImportedPlugin>) => {
    ...
    const persisted = await writeWorkspaceOpenworkConfigRecord(nextConfig);
    if (!persisted) {
      throw new Error("RenWork server unavailable. Connect to manage imported cloud plugins.");
    }
  ...
  const deleteWorkspaceSkill = async (name: string) => {
    ...
    if (hasOpenworkTarget) {
      throw new Error("RenWork server cannot remove skills for this workspace.");
    }

    if (isRemoteWorkspace) {
      throw new Error("RenWork server unavailable. Connect to remove skills.");
    }
  ...
  const upsertPluginMcpConfig = async (name: string, config: Record<string, unknown>) => {
    ...
    throw new Error("RenWork server unavailable. Connect to import MCP servers into this workspace.");
  };
  ...
  const deletePluginMcpConfig = async (name: string) => {
    ...
    throw new Error("RenWork server unavailable. Connect to remove imported MCP servers from this workspace.");
  };
  ...
  const writePluginWorkspaceFile = async (path: string, content: string) => {
    ...
    throw new Error("RenWork server unavailable. Connect to import plugin files into this workspace.");
  };
  ...
  const deletePluginWorkspaceFiles = async (files: Array<{ path: string; recursive?: boolean }>) => {
    ...
    throw new Error("RenWork server unavailable. Connect to remove imported plugin files from this workspace.");
  };
  ...
  async function importCloudOrgPlugin(
    ...
    try {
      ...
      const token = settings.authToken?.trim() ?? "";
      const orgId = settings.activeOrgId?.trim() ?? "";
      if (!token || !orgId) throw new Error("Sign in to RenWork Cloud and choose an organization first.");
      const client = createDenClient({ baseUrl: settings.baseUrl, token });
  ...
  async function previewClaudePlugin(url: string): Promise<OpenworkClaudePluginPreview> {
    const target = await resolveWorkspaceServerTarget();
    if (!target.openworkClient || !target.openworkWorkspaceId) {
      throw new Error("RenWork server unavailable. Connect to install plugins from GitHub.");
    }
  ...
  async function refreshSkills(optionsOverride?: { force?: boolean }) {
    ...
    if (hasOpenworkTarget) {
      mutateState((current) => ({
        ...current,
        skills: [],
        skillsStatus: "RenWork server cannot read skills for this workspace.",
      }));
      return;
    }
    ...
    if (!client) {
      mutateState((current) => ({
        ...current,
        skills: [],
        skillsStatus: "RenWork server unavailable. Connect to load skills.",
      }));
      return;
    }
  ...
  async function refreshPlugins(scopeOverride?: PluginScope) {
    ...
    if (scope === "project" && hasOpenworkTarget) {
      mutateState((current) => ({
        ...current,
        pluginStatus: "RenWork server cannot read plugins for this workspace.",
        pluginList: [],
        sidebarPluginStatus: "RenWork server cannot read plugins for this workspace.",
        sidebarPluginList: [],
      }));
      refreshPluginsInFlight = false;
      return;
    ...
    if (!isLocalWorkspace && !canUseOpenworkServer) {
      mutateState((current) => ({
        ...current,
        pluginStatus: "RenWork server unavailable. Connect to manage plugins.",
        pluginList: [],
        sidebarPluginStatus: "Connect an RenWork server to load plugins.",
        sidebarPluginList: [],
      }));
      refreshPluginsInFlight = false;
      return;
  ...
  async function addPlugin(pluginNameOverride?: string) {
    ...
    if (snapshot.pluginScope === "project" && hasOpenworkTarget) {
      setStateField("pluginStatus", "RenWork server cannot write plugins for this workspace.");
      return;
    }
    ...
    if (!isLocalWorkspace) {
      setStateField("pluginStatus", "RenWork server unavailable. Connect to manage plugins.");
      return;
    }
  ...
  async function installSkillCreator(): Promise<{ ok: boolean; message: string }> {
    ...
    if (hasOpenworkTarget) {
      const message = "RenWork server cannot write skills for this workspace.";
      setStateField("skillsStatus", message);
      return { ok: false, message };
    }

    if (isRemoteWorkspace) {
      const message = "RenWork server unavailable. Connect to install skills.";
      setStateField("skillsStatus", message);
      return { ok: false, message };
    }
    if (!isDesktopRuntime()) {
  ...
  async function readSkill(name: string): Promise<{ name: string; path: string; content: string } | null> {
    ...
    if (hasOpenworkTarget) {
      setStateField("skillsStatus", "RenWork server cannot read skills for this workspace.");
      return null;
    }
    ...
    if (isRemoteWorkspace) {
      setStateField("skillsStatus", "RenWork server unavailable. Connect to view skills.");
      return null;
    }
    if (!isDesktopRuntime()) {
      setStateField("skillsStatus", t("skills.desktop_required"));
  ...
  async function saveSkill(input: { name: string; content: string; description?: string }) {
    ...
    if (hasOpenworkTarget) {
      setStateField("skillsStatus", "RenWork server cannot write skills for this workspace.");
      return;
    }
    ...
    if (isRemoteWorkspace) {
      setStateField("skillsStatus", "RenWork server unavailable. Connect to edit skills.");
      return;
    }
    if (!isDesktopRuntime()) {
      setStateField("skillsStatus", t("skills.desktop_required"));
