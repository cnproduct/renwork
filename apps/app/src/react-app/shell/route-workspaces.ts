...
export function describeWorkspaceCreateError(error: unknown) {
  ...
  if (
    ...
    return `${message}\n\nRenWork could not read the workspace config before the filesystem timed out. This often happens when the folder is still syncing from iCloud Drive or another remote folder. Wait for the folder to finish downloading, move the workspace to a local folder, or try again.`;
  }
  return message;
}
...
export function mergeRouteWorkspaces(
  ...
  // skip the server's view entirely. The local RenWork server may have stale
