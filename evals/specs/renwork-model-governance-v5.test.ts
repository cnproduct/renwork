...
test("V5 separates platform provider administration from organization model policy", async ({ evidence }) => {
  ...
  expect(session).toContain("allowProviderManagement={localProviderManagementAllowed}");
  ...
  evidence.fact("Cloud model selection is read-only", "Members and organization Owners can choose only published models; local Ollama and BYOK management remains available only to an independent signed-out local profile.", true);
