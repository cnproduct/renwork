...
test("V13 unifies RenWork identity, version governance, RBAC and durable settlement audit", async ({ evidence }) => {
  ...
  expect(new Set([rootPackage.version, appPackage.version, desktopPackage.version, serverPackage.version])).toEqual(new Set(["0.18.61"]));
  ...
  evidence.fact("Release version is single-source consistent", "Root, web app, desktop and embedded server all report 0.18.61.", true);
