...
test("the post-sign-in welcome screen offers only entitled RenWork managed models", async ({ evidence }) => {
  ...
  expect(welcomeSource).not.toContain('data-testid="welcome-model-source-local"');
  expect(welcomeSource).not.toContain('data-testid="welcome-model-source-byok"');
  expect(welcomeSource).toContain('data-testid="welcome-model-source-continue"');
  expect(welcomeSource).toContain("RenWork 官方模型");
  ...
  evidence.fact(
    ...
    "Members continue with RenWork managed models only; organization invite parsing remains in the dedicated join dialog and no local-model or BYOK shortcut is exposed.",
    true,
  );
});
