...
test("the welcome screen folds the server-URL door into the join-organization field", async ({ evidence }) => {
  ...
  evidence.fact(
    ...
    "The separate Using RenWork on-premises affordance is gone from Welcome; the join dialog classifies install link, then invite link, then server URL, then sign-in code, and requires explicit host confirmation before opening web invites.",
    true,
  );
});
