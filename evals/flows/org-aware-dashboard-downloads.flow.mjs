...
export default {
  ...
  steps: [
      ...
      run: async (ctx) => {
        await ctx.prove("The admin dashboard offers Acme's configured installer instead of public release assets", {
          ...
          action: async () => {
            ...
            await ctx.waitForText(`Download RenWork for ${ORGANIZATION_NAME}`, { timeoutMs: 30_000 });
          },
          assert: async () => {
            await ctx.expectText(`Download RenWork for ${ORGANIZATION_NAME}`);
          ...
          screenshot: {
            ...
            requireText: [`Download RenWork for ${ORGANIZATION_NAME}`, "Download for this workspace"],
      ...
      run: async (ctx) => {
        await ctx.prove("Alex reaches an Acme install page that clearly requires sign-in", {
          ...
          screenshot: {
            ...
            requireText: [`Download RenWork for ${ORGANIZATION_NAME}`, "Run it, then sign in", "Download for"],
          },
        });
      },
    },
      ...
      run: async (ctx) => {
        await ctx.prove("An ordinary member sees the download action without admin distribution controls", {
          ...
          screenshot: {
            ...
            requireText: ["WORKSPACE MEMBER", `Download RenWork for ${ORGANIZATION_NAME}`, "Download for this workspace"],
    ...
    },
    {
      name: "Frame 4",
      run: async (ctx) => {
        await ctx.prove("Riley receives the configured Windows installer from Acme's RenWork server", {
          voiceover: vo[3],
          action: async () => {
          ...
          screenshot: {
            ...
            requireText: [`Download RenWork for ${ORGANIZATION_NAME}`, "Download for Windows", "Run it, then sign in"],
          },
        });
      },
    },
      ...
      run: async (ctx) => {
        await ctx.prove("Possessing the installer config does not grant Acme workspace access", {
          ...
          screenshot: {
            ...
            requireText: [`Download RenWork for ${ORGANIZATION_NAME}`, "Run it, then sign in"],
