...
async function runInjectedSessionTools(baseUrl) {
  const script = `
    ...
    const search = JSON.parse(await plugin.tool.renwork_query.execute({ id: "session.search", args: { query: "raven launch", limit: 5, scanLimit: 10 } }));
    const read = JSON.parse(await plugin.tool.renwork_query.execute({ id: "session.read", args: { sessionId: "ses_archive", count: 2 } }));
  ...
  steps: [
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        try {
          ...
          await ctx.prove("renwork_query session.search returns a transcript hit from another chat", {
            action: async () => {
              const hit = output.search.results[0];
              await showProofPanel(ctx, "Past Chat Search Tool Proof", [
                { label: "Tool called", value: "renwork_query session.search" },
            ...
            screenshot: { name: "session-search-hit", requireText: ["renwork_query session.search", "raven launch", "ses_alpha"] },
          });
        } finally {
          await server.close();
        }
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        await ctx.prove("renwork_query session.read retrieves the referenced session transcript", {
          action: async () => {
            await showProofPanel(ctx, "Past Chat Read Tool Proof", [
              { label: "Tool called", value: "renwork_query session.read" },
          ...
          screenshot: { name: "session-read-transcript", requireText: ["renwork_query session.read", "Archive decisions", "ship the archive importer first"] },
        });
      },
    },
  ],
