...
const CLIENT_NAME = "RenWork eval URL-only MCP client";
...
let loopbackPage = {
  ...
  details: ["RenWork MCP client is waiting for the browser callback."],
};
...
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <title>RenWork MCP client — ${escapeHtml(loopbackPage.status)}</title>
    <style>
  ...
  <body>
    <main>
      ...
      <h1>RenWork MCP client — ${escapeHtml(loopbackPage.status)}</h1>
...
async function startLoopbackServer() {
  ...
  setLoopbackPage("waiting for authorization", ["RenWork MCP client is waiting for the browser callback."]);
  let resolved = false;
  let resolveCode;
  let rejectCode;
  ...
  const server = http.createServer((request, response) => {
    ...
    if (requestUrl.pathname === "/callback") {
      ...
      if (code) {
        setLoopbackPage("authorization code received", ["RenWork redirected back to the loopback callback.", "The client can now exchange the code for tokens."]);
        if (!resolved) {
          resolved = true;
          resolveCode(code);
        }
      ...
      response.end("<h1>RenWork MCP client — missing code</h1>");
      return;
    }
      ...
      run: async (ctx) => {
        await ctx.prove(vo[2], {
          ...
          assert: async () => {
            ...
            recordAssertion(
              ctx,
              "The authorize URL sends the browser to the real RenWork sign-in page",
