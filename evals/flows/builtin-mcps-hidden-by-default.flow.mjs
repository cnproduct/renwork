...
export default {
  ...
  title: "Built-in RenWork MCPs are hidden by default and revealed by Show hidden",
  ...
  steps: [
    ...
    {
      name: "Built-in RenWork MCPs are hidden by default",
      run: async (ctx) => {
        const directoryEntry = await ctx.hasText("Notion") ? "Notion" : "Linear";
        await ctx.prove("Built-in RenWork MCPs are hidden from the default extensions list", {
          voiceover: "The regular MCP directory still renders in Settings, but RenWork's internal control entries are not listed by default.",
          assert: async () => {
          ...
          screenshot: {
            ...
            claim: "RenWork Cloud Control and RenWork UI Control are hidden from the default extensions list while the rest of the catalog renders.",
            requireText: ["Show hidden", directoryEntry],
            rejectText: ["RenWork Cloud Control", "RenWork UI Control", "Something went wrong"],
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("Show hidden reveals both built-in RenWork MCP entries", {
          voiceover: "When the user asks to show hidden extensions, both built-in RenWork MCP controls appear for inspection or management.",
          action: async () => {
            await revealHidden(ctx);
          },
          assert: async () => {
            await ctx.expectText("RenWork Cloud Control", { timeoutMs: 30_000 });
            await ctx.expectText("RenWork UI Control", { timeoutMs: 30_000 });
            ...
            await ctx.eval(`(() => {
              ...
              const card = buttons.find((el) => (el.textContent ?? "").includes("RenWork UI Control"));
          ...
          screenshot: {
            ...
            claim: "Show hidden reveals RenWork Cloud Control and RenWork UI Control with hidden styling.",
            requireText: ["RenWork Cloud Control", "RenWork UI Control", "Showing hidden"],
