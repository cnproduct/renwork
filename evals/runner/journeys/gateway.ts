...
export const hostileGatewayFaultCases: readonly HostileGatewayFaultCase[] = [
  {
    ...
    claim: "RenWork's real connect path shows today's redirect-whitelist surface, including the exact generated callback but not the provider's rejected-URI message.",
  ...
  },
  {
    ...
    claim: "RenWork's real callback path reaches token exchange and currently collapses a per-connector redirect failure to the generic token-exchange diagnostic.",
  ...
  },
  {
    ...
    claim: "RenWork's real connect path attempts dynamic client registration when advertised, then currently maps an authorization-time DCR-required error generically.",
  ...
  {
    faultId: "method-405",
    claim: "RenWork's real connect path does not issue a GET to the MCP endpoint, so a GET-only 405 fault is latent in this user flow.",
    expectedProductSnippets: ["GET /mcp count 0", "needs_auth"],
  },
  {
    faultId: "duplicate-amplification",
    claim: "RenWork sends one tools/call for one user action, but today's Den diagnostic loses the duplicate-upstream-request signal.",
    expectedProductSnippets: ["one MCP tools/call", "provider status 409", "MCP_PROVIDER_TOOL_ERROR"],
  },
  {
    faultId: "refresh-expired",
    claim: "RenWork's real runtime attempts a refresh-token grant for expired credentials and currently reports a generic token/refresh exchange rejection.",
  ...
  },
  {
    ...
    claim: "RenWork's real client canonicalizes the trailing-dot authorization server before issuer discovery and still reaches authorization.",
    expectedProductSnippets: ["canonicalized", "needs_auth", "trailingDotRequests"],
  },
  {
    faultId: "per-user-403",
    claim: "RenWork's real callback validation classifies a per-user 403 as provider authorization, but today's message does not name the subject.",
