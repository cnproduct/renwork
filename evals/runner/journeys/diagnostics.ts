...
export async function expectVerdictNames(
  ...
  ctx.output(`${options.lab.profile} RenWork product diagnostics verdict`, `${verdict.text}\n\n${verdict.evidence}`);
  ctx.output(`${options.lab.profile} lab-local corroborating probes`, `${corroboration.text}\n\n${corroboration.evidence}`);
  evidence(
    ctx,
    verdict.available && matched.ok,
    `RenWork product diagnostics verdict did not name expected fault(s): ${matched.missing.join(", ")}. Verdict: ${verdict.text}`,
    verdict.text,
  );
  return verdict;
}
...
export async function expectRuntimeTrust(ctx: FlowContext, options: { caPath: string }): Promise<void> {
  const env: NodeJS.ProcessEnv = {
    ...
    OPENWORK_TLS_REPRO_CA_MATCH: "RenWork Egress Lab",
  };
