  ...
  try {
    ...
    if (faultId === "redirect-uri-whitelist" || faultId === "dcr-required") {
      ...
      const finding = {
        ...
        actionableImmediately: faultId === "dcr-required"
          ? "Partially — RenWork does perform dynamic registration when the provider advertises it, but an authorization-time DCR-required error still surfaces as a generic provider authorization rejection."
        ...
        assertions: [
          ...
          assertion("Product authorize URL carries the exact RenWork redirect URI", connect?.redirectUri === redirectUri, { redirectUri, productRedirectUri: connect?.redirectUri ?? null }),
    ...
    if (faultId === "method-405") {
      ...
      return {
        ...
        assertions: [
          ...
          assertion("One real RenWork connect action produced zero GET requests to the MCP endpoint", counts.getMcpEndpoint === 0, counts),
        ],
        behavior: "connectExternalMcp returned needs_auth; the hostile GET-only 405 behavior is latent because RenWork's client uses POST for MCP initialize and never GETs the endpoint in this flow.",
        connect,
    ...
    if (faultId === "trailing-dot-url") {
      ...
      return {
        ...
        assertions: [
          assertion("RenWork did not propagate a trailing-dot authority into product fetches", counts.trailingDotRequests === 0, counts),
    ...
    if (faultId === "duplicate-amplification") {
      ...
      return {
        ...baseFinding({ connection, faultId, fileReferences, profile, server }),
        actionableImmediately: "No — one RenWork tool action sends exactly one MCP tools/call, but the returned product diagnostic does not preserve the provider's duplicate-request signal.",
        assertions: [
          assertion("One user tool action produced exactly one RenWork tools/call request", counts.mcpToolCalls === 1, counts),
