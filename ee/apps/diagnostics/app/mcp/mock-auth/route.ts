...
export async function GET(request: Request): Promise<Response> {
  ...
  if (!subject) return html(400, "Verification link expired", "Return to RenWork and run the diagnostics authorization check again to receive a fresh link.")
  try {
  ...
  return html(200, "Diagnostics authorization complete", "Return to RenWork and ask the agent to retry the diagnostics authorization check. This mock authorization resets automatically after five minutes.")
}
