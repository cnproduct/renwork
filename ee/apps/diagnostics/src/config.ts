...
export function diagnosticsConfig() {
  ...
  return {
    adminPassword: process.env.DIAGNOSTICS_ADMIN_PASSWORD ?? (hosted ? "" : "RenWorkDiagnosticsLocal!"),
    adminUsername: process.env.DIAGNOSTICS_ADMIN_USERNAME ?? "diagnostics-admin",
    bearerToken: process.env.DIAGNOSTICS_MCP_BEARER_TOKEN ?? (hosted ? "" : "RenWorkDiagnosticsToken!"),
