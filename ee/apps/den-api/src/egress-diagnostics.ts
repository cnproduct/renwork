...
function httpFailure(status: number, category: EgressDiagnosticCategory): DiagnosticFailure {
  ...
  if (status >= 500) {
    return new DiagnosticFailure({
      action: "Give RenWork support the run ID and diagnostic reference so the Diagnostics deployment can be inspected.",
      category,
      code: `http_${status}`,
  ...
  return new DiagnosticFailure({
    action: "Give RenWork support the run ID and response status, then compare the matching remote trace.",
    category,
    code: `http_${status}`,
...
async function readJson(response: Response): Promise<Record<string, unknown>> {
  ...
  if (!reader) throw new DiagnosticFailure({
    action: "Give RenWork support the run ID and diagnostic reference.",
    category: "http",
  ...
  while (true) {
    ...
    if (bytes > maximumBytes) {
      ...
      throw new DiagnosticFailure({
        action: "Give RenWork support the run ID; the diagnostic response exceeded its declared safety bound.",
        category: "http",
...
function protocolFailure(category: EgressDiagnosticCategory, code: string, message: string): DiagnosticFailure {
  return new DiagnosticFailure({
    action: "Give RenWork support the run ID and diagnostic reference so the response contract can be compared with the remote trace.",
    category,
    code,
    message,
    owner: "openwork-support",
  ...
  const tasks: Record<EgressDiagnosticStepId, (evidence: StepEvidence) => Promise<void>> = {
    ...
    "mcp-handshake": async (evidence) => {
      ...
      requireValue(result !== null && result.protocolVersion === "2025-11-25" && session.length > 20 && version === "2025-11-25", {
        action: "Give RenWork support the run ID and initialize diagnostic reference.",
      ...
      requireValue(catalogResult !== null && Array.isArray(catalogResult.tools) && catalogResult.tools.length === 1, {
        action: "Give RenWork support the run ID and catalog diagnostic reference.",
      ...
      requireValue(toolResult !== null && toolResult.isError === false, {
        action: "Give RenWork support the run ID and tool-call diagnostic reference.",
