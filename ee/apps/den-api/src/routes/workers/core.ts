...
export function registerWorkerCoreRoutes<T extends { Variables: WorkerRouteVariables }>(app: Hono<T>) {
  ...
  app.post(
    ...
    if (input.destination === "cloud") {
      ...
      const access = await requireCloudAccessOrPayment({
        ...
        email,
        name: user.name ?? user.email ?? "RenWork User",
      })

      if (!access.allowed) {
        return c.json({
          error: "cloud_worker_billing_unavailable",
          message: "Creating new cloud workers requires an existing RenWork Cloud plan. New self-serve purchases are no longer available.",
        }, 402)
      }
  ...
  app.post(
    ...
    describeRoute({
      ...
      description: "Returns connection tokens and the resolved RenWork connect URL for an existing worker.",
      responses: {
