    ...
    async (c) => {
      ...
      if (c.get("session")?.id === "mcp_internal") {
        return c.json({
          ...
          message: "Telegram bot tokens cannot be set from an agent. Add the token in RenWork Cloud Connect.",
        }, 400)
      }
      ...
      try {
        ...
        return c.json({ error: "invalid_request", message: "Select a valid RenWork worker." }, 400)
      }
      ...
      try {
        ...
        if (isDuplicateDatabaseEntry(error)) {
          return c.json({
            ...
            message: "This Telegram bot is already connected to another RenWork workspace.",
          }, 409)
        }
