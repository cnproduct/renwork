...
const DEFAULT_OPENWORK_EXTENSION_MANIFESTS = [
  {
    ...
    setup: { instructions: "RenWork Browser is ready by default in desktop workspaces." },
  ...
  },
  {
    ...
    setup: { instructions: "Add an OpenAI API key, then agents can generate image artifacts through RenWork extension actions." },
    resources: [
  ...
  },
  {
    ...
    description: "Let RenWork help with meetings, selected Drive files, and Gmail drafts.",
    source: { format: "openwork-builtin", origin: "builtin", trusted: true },
    icon: { simpleIconSlug: "google" },
    composer: { prompt: "Use Google Workspace to " },
    setup: { instructions: "Connect your Google account to use Calendar, Drive, and Gmail drafts in RenWork." },
    resources: [
...
function serializePluginExtension(row: PluginRow, componentCounts: Record<string, number>) {
  ...
  return {
    ...
    manifest: {
      ...
      setup: {
        instructions: "Imported from a Claude-compatible plugin. RenWork installs its resources into this workspace as extension components.",
      },
      lifecycle: {
