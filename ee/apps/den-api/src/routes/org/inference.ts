...
const renCreditTaskReceiptSchema = z.object({
  ...
  platform_price_multiplier_bps: z.number().int().nonnegative().nullable(),
  organization_multiplier_override_bps: z.number().int().nonnegative().nullable(),
  effective_price_multiplier_bps: z.number().int().nonnegative().nullable(),
  pricing_policy_version: z.number().int().nonnegative().nullable(),
