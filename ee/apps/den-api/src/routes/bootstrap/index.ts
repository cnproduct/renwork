...
const bootstrapWorkspaceSchema = z.object({
  ...
  skillName: z.string().trim().min(1).max(120).default("First RenWork Skill"),
...
function skillMetadata(skillText: string) {
  ...
  if (parsed.hasFrontmatter) {
    return {
      ...
      description: (parsed.description.trim() || "Starter skill created by RenWork agent bootstrap.").slice(0, 65535),
    }
  }
