  ...
  await consumeAndConfirmTelegramPairing({
    ...
    confirm: async (pairing) => {
      await sendForCurrentConnection({
        ...
        text: pairing.paired
          ? "Connected. Messages in this private chat will now go to your selected RenWork worker."
          : "This pairing link is invalid, expired, or already used. Create a new link in RenWork Connect.",
      })
    },
  })
}
...
async function processTelegramUpdate(input: {
  ...
  if (/^\/start(?:@[A-Za-z0-9_]+)?$/.test(text)) {
    await sendForCurrentConnection({
      ...
      dispatchToken: input.dispatchToken,
      generation: input.generation,
      text: "Open RenWork Connect and create a pairing link for this bot, then use that link here.",
    })
    await updateStatus("completed")
    return
  }
  ...
  if (!binding || binding.telegramChatId !== chatId || binding.telegramUserId !== String(sender.id)) {
    await sendForCurrentConnection({
      ...
      dispatchToken: input.dispatchToken,
      generation: input.generation,
      text: "This private chat is not paired with RenWork. Create a new pairing link in RenWork Connect.",
    })
    await updateStatus("ignored")
    return
  }
  ...
  try {
    ...
    const notice = error instanceof TelegramWorkerTimeoutError
      ? "The worker is still waiting. Check RenWork for a permission or question, then try again."
      : "I couldn't reach the selected RenWork worker. Check its status in RenWork and try again."
    try {
      await sendForCurrentConnection({
        chatId,
        connectionId: input.connection.id,
