...
export async function provisionWorker(
  ...
  if (env.provisionerMode === "self_hosted") {
    return provisionWorkerOnSelfHosted(input)
  }
...
export async function deprovisionWorker(input: {
  ...
  if (env.provisionerMode === "self_hosted") {
    await deprovisionWorkerOnSelfHosted(input.workerId)
    return
  }
