...
function isOAuthTokenPhase(phase: ExternalMcpDiagnosticPhase): boolean {
  return phase === "AUTH_TOKEN_ACQUISITION" || phase === "CONTINUITY_REFRESH"
}
...
function errorName(value: unknown): string {
  const name = value instanceof Error && value.name === "Error" && SAFE_ERROR_NAMES.has(value.constructor.name)
...
function safeBaseMessageFor(input: {
  ...
  if (input.code === "MCP_LIFECYCLE_DEADLINE") {
    return input.phase === "MCP_TOOL_EXECUTION"
      ? "The capability did not finish within the time RenWork allows a single tool call."
      : "The MCP lifecycle exceeded RenWork's bounded diagnostic deadline."
  }
  ...
  if (input.code === "MCP_PROVIDER_DECLARED_ERROR") {
    const message = input.jsonRpcCode === undefined
      ? "The provider answered with a JSON-RPC error RenWork does not recognize."
      : `The provider answered with a JSON-RPC error RenWork does not recognize (code ${input.jsonRpcCode}).`
    return input.providerErrorMessage
      ? `${message} Provider-declared message (untrusted): "${input.providerErrorMessage}".`
      : message
  }
  if (input.code === "MCP_RESPONSE_BODY_LIMIT") {
    return "The MCP server returned a response body larger than RenWork can safely process."
  }
  if (input.highestPassed && HEALTH_RANK[input.highestPassed] >= HEALTH_RANK.protocol_ready && isUninformativeClassification(input)) {
    return "The MCP server answered, but RenWork could not interpret its response for the current request."
  }
  ...
  if (input.phase === "AUTH_CLIENT_REGISTRATION") {
    return "RenWork could not register or identify its OAuth client with the authorization server."
  }
  ...
  if (input.phase === "MCP_TOOL_DISCOVERY") {
    return input.code === "MCP_CATALOG_CURSOR_LOOP"
      ? "The MCP server repeated a tool-catalog cursor, so RenWork stopped safely."
      : "RenWork could not retrieve a complete, valid MCP tool catalog."
  }
  ...
  return "The MCP connection failed before RenWork could complete the protocol lifecycle."
}
...
function classifyError(error: unknown, fallbackPhase: ExternalMcpDiagnosticPhase): Classification {
  ...
  if (hasUnsupportedVersionMessage(error)) {
    return {
      ...
      retryable: false,
      actionOwner: "provider_admin",
      operatorAction: "Configure the provider to support an MCP protocol version compatible with RenWork.",
    }
  }
...
export class ExternalMcpDiagnosticTracker {
  ...
  error(error: unknown, fallbackPhase = this.lastFailedPhase ?? this.phase): ExternalMcpDiagnosticError {
    ...
    const safeOAuthProviderDetail = oauthProviderDetail(error)
      ...
      ?? capturedOAuthProviderDetail
    const providerErrorMessage = providerEvidence?.message ?? safeOAuthProviderDetail
    ...
    const diagnostic: ExternalMcpDiagnostic = {
      ...
      message: safeMessageFor({
        ...diagnosticWithoutMessage,
        ...(safeOAuthProviderDetail ? { providerDetail: safeOAuthProviderDetail } : {}),
      }),
...
async function responseTextExcerptWithinLimit(response: Response, limit: number): Promise<string | null> {
  if (!response.body) return null
  const reader = response.body.getReader()
