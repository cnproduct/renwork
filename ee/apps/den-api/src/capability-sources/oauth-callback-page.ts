...
export function connectCallbackPage(input:
  ...
  const body = input.ok
    ? `<div class="status-row success">
        ...
        <span><strong>${escapeHtml(input.name)} is connected to RenWork.</strong><small>Connection complete</small></span>
      </div>
      <p>You can close this window and return to RenWork.</p>
      ${closeButton}`
...
<html lang="en">
  ...
  <body>
    <main role="${input.ok ? "status" : "alert"}" aria-live="${input.ok ? "polite" : "assertive"}">
      <p class="eyebrow">RenWork Connect</p>
      <h1>${title}</h1>
      ${body}
    </main>
  </body>
