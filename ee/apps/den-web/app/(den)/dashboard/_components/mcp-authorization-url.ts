...
<html lang="en">
  <head>
    ...
    <title>Connecting — RenWork</title>
    <style>
${authorizationDocumentStyles}
  ...
  <body>
    <main class="card connect-card" role="status" aria-live="polite">
      <div class="brand"><span class="brand-mark">OW</span>RenWork Connect</div>
      <h1>Connect your account</h1>
      <p>RenWork is preparing the secure provider sign-in.</p>
...
<html lang="en">
  <head>
    ...
    <meta name="viewport" content="width=device-width, initial-scale=1" />
    <meta name="color-scheme" content="light" />
    <title>Connection failed — RenWork</title>
    <style>
${authorizationDocumentStyles}
    ...
    <main class="card error-card" role="alert" aria-live="assertive">
      <div class="error-header">
        ...
        <div class="status">
          ...
          <p>RenWork couldn’t start the provider sign-in.</p>
        </div>
...
export function openMcpAuthorizationWindow(): Window {
  ...
  if (!popup) {
    throw new Error("RenWork could not open the sign-in window. Allow popups for RenWork, then try again.")
  }
  try {
    popup.opener = null
    popup.document.open()
